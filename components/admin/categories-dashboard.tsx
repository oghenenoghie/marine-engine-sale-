...
import { saveCategoryAction, deleteCategoryAction } from "@/lib/actions/taxonomy";
import type { PartCategory } from "@/types";

const uid = () => (typeof crypto !== "undefined" ? crypto.randomUUID() : String(Math.random()).slice(2));

export function CategoriesDashboard({ initialCategories }: { initialCategories: PartCategory[] }) {
  const [categories, setCategories] = useState(initialCategories);
  const [editing, setEditing] = useState<PartCategory | Partial<PartCategory> | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<PartCategory | null>(null);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const save = async (category: PartCategory) => {
    setSaving(true);
    setError(null);
    const result = await saveCategoryAction(category);
    if (result.ok) {
      setCategories((prev) =>
  ...
  return (
    <div className="text-[14px] text-hull">
      ...
      <div className="mb-5 flex items-center justify-between">
        <div>
          <div className="font-display text-xl font-extrabold tracking-tight">Categories</div>
          <div className="text-[12px] text-steel">Part categories used across stock items</div>
        </div>
      ...
      {editing !== null && (
        <CategoryForm initial={editing} saving={saving} onClose={() => setEditing(null)} onSave={save} />
      )}
      {deleteTarget && (
        <ConfirmDelete
...
function CategoryForm({
  ...
  initial: PartCategory | Partial<PartCategory>;
  saving: boolean;
  onClose: () => void;
  onSave: (category: PartCategory) => void;
}) {
  ...
  const handleSave = () => {
    if (!canSave) return;
    onSave({ id: initial.id ?? uid(), name: name.trim(), slug: slug.trim() || slugify(name), parentId: initial.parentId ?? null });
  };
    ...
    <div className="fixed inset-0 z-40 flex justify-end bg-hull/45" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col bg-paper shadow-2xl">
        <div className="flex items-center justify-between border-b border-steel/15 px-5 py-4">
          <div className="font-display text-lg font-extrabold tracking-tight">{isNew ? "New category" : "Edit category"}</div>
